...
# ==========================================
INPUT_FILE = '5.csv'
OUTPUT_FILE = 'synthetic_datasets/bad_datasets/datasets_without_marks/synthetic_datasets_bad_contact/synthetic_dataset_bad_contact5.csv'
N_ROWS = 10000
...
def inject_anomalies(df, target_cols):
    ...
    for col in target_cols:
        ...
        # --- Тип 1: Плохой контакт (Шум) ---
        # 5-8 участков. Длительность: 50-300 мс (3-15 отсчетов при шаге 20 мс)
        for _ in range(np.random.randint(5, 8)):
            idx = np.random.randint(0, n - 20)
            length = np.random.randint(3, 16)
            # Хаотичные пики амплитудой 5-20%
            noise_amp = mean_val * np.random.uniform(0.05, 0.2)
            signal[idx:idx + length] += np.random.normal(0, noise_amp, length)
            labels[idx:idx + length] = np.maximum(labels[idx:idx + length], 1)

        # --- Тип 2: ВЧ шум (другая амплитуда) ---
        # 3-5 участков. Шум составляет 10-30% от амплитуды
        for _ in range(np.random.randint(3, 6)):
            idx = np.random.randint(0, n - 20)
            length = np.random.randint(3, 20)
            # Хаотичные пики амплитудой 30-50% (сейчас выбраны от 15 до 40% от амплитуды)
            noise_amp = mean_val * np.random.uniform(0.1, 0.3)
            signal[idx:idx + length] += np.random.normal(0, noise_amp, length)
            labels[idx:idx + length] = np.maximum(labels[idx:idx + length], 1)

        # --- Тип 3: Те же шумы но с амплитудой 20-45%
        for _ in range(np.random.randint(2, 5)):
            idx = np.random.randint(0, n - 20)
            length = np.random.randint(3, 16)
            # Хаотичные пики амплитудой 30-50% (сейчас выбраны от 15 до 40% от амплитуды)
            noise_amp = mean_val * np.random.uniform(0.20, 0.45)
            signal[idx:idx + length] += np.random.normal(0, noise_amp, length)
            labels[idx:idx + length] = np.maximum(labels[idx:idx + length], 1)
