...
# Преобразование: 0 (норма) -> 1; 1,2,3 (аномалия) -> 0
...
model_files = [f'{base_dir}synthetic_dataset_bad_contact{i}.csv' for i in range(1, 3)]
...
for i, file in enumerate(model_files, start=1):
    ...
    # ПРЕОБРАЗУЕМ модельные метки в ту же шкалу: 1 – норма, 0 – аномалия
    # Предполагаем, что в модельном файле Anomaly_Class: 0=норма, 1,2,3=аномалия
    pred_col = 'Anomaly_Class' if 'Anomaly_Class' in df_m.columns else 'prediction'
    df_m['model_label'] = df_m[pred_col].apply(lambda x: 1 if x == 0 else 0)

    df_m = df_m[['TimeStamp', 'model_label']].rename(columns={'model_label': f'pred_{i}'})
    model_dfs.append(df_m)

# Объединение всех модельных предсказаний
...
# ==========================================
# 3. Слияние с истиной и итоговый вердикт
# ==========================================
df_final = pd.merge(df_true, df_models, on='TimeStamp', how='left')
pred_cols = [f'pred_{i}' for i in range(1, len(model_dfs) + 1)]
df_final[pred_cols] = df_final[pred_cols].fillna(1)  # нет предсказания – считаем нормой

# Сумма предсказаний (каждое =1 для нормы, 0 для аномалии)
df_final['sum_pred'] = df_final[pred_cols].sum(axis=1)

# Логика: аномалия (0), если хотя бы одна модель сказала 0 (аномалия)
# Иначе норма (1)
df_final['final_model_label'] = (df_final['sum_pred'] == len(model_dfs)).astype(int)
